...
export const AdminVeterinarias: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const res = await adminService.listVeterinarias({ page, limit, q });
            const data = res?.data ?? res;
            setItems(data?.veterinarias ?? []);
            setMeta(data?.meta ?? null);
        } catch (err) {
            console.error("admin veterinarias list error", err);
            setItems([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("¿Eliminar veterinaria? Esta acción es irreversible."))
            return;
        try {
            ...
            console.error(err);
            alert("No se pudo eliminar la veterinaria");
        }
    };
                ...
                <div className="mx-auto max-w-5xl">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Veterinarias</h1>
                                <p className="text-sm text-slate-300">Gestión de veterinarias adheridas</p>
                            </div>
                        </div>
                        <button
                                    ...
                                    {filteredItems.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="px-4 py-12 text-center">
                                                ...
                                                <p className="text-sm text-gray-400">No hay veterinarias para mostrar</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
